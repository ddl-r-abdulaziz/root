...
# render2 downloads this bundle's capabilities (root has no imports, so just
# its own) into their own subdirectory here.
...
.PHONY: configure
configure: $(RENDER2_OUTPUT) ## Apply base to an agent.yaml (stdin) using configure
